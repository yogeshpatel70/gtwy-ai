...
async def cleanup_tool_count_after(task, key: Optional[str], owner_token: Optional[str]) -> None:
    """Defer ``cleanup_tool_count`` until the spawned streaming task finishes.

    Streaming requests return a ``StreamingResponse`` synchronously after
    scheduling ``sse_stream_and_finalize`` as a background task. The caller's
    own ``finally`` would otherwise wipe the counter before the loop runs.
    """
    try:
        await task
    except Exception:
        ...
        cleanup_tool_count(key, owner_token)
