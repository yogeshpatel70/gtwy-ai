...
class NeevCloud(BaseService):
    async def execute(self):
        ...
        if self.stream_mode:
            neevCloudResponse = await self.stream(self.customConfig, self.apikey, service_name["neev_cloud"])
        else:
            neevCloudResponse = await self.chats(self.customConfig, self.apikey, service_name["neev_cloud"])
        modelResponse = neevCloudResponse.get("modelResponse", {})
        if not neevCloudResponse.get("success"):
            await self.handle_failure(neevCloudResponse)
            raise ValueError(neevCloudResponse.get("error"))
        if len(modelResponse.get("choices", [])[0].get("message", {}).get("tool_calls", [])) > 0:
            functionCallRes = await self.function_call(
                self.customConfig, service_name["neev_cloud"], neevCloudResponse, 0, {}
            )
        ...
        response = await Response_formatter(
            modelResponse, service_name["neev_cloud"], tools, self.type, self.image_data
        )
